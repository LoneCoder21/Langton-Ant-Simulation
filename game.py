...
class Game:
    ...
    def createAnts(self, amount):
        ...
        for i in range(amount):
            self.ants.append(Ant((random.randint(0, Default.GW - 1), random.randint(0, Default.GH - 1))))

    def randomizeAnts(self):
        for i in range(len(self.ants)):
            self.ants[i] = Ant((random.randint(0, Default.GW - 1), random.randint(0, Default.GH - 1)))
